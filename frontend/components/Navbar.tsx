  ...
  return (
    <div>
      <div className="flex gap-4">
        {!(accounts.current?.length > 0) && currentWallet && (
...
{ !currentWallet && !(accounts.current?.length > 0) && (
        <button onClick={()=>{setloginbox(true)}} className="border border-white py-2 px-10 bg-white text-black rounded-lg">Login</button>
)}
        ...
        >
          <div className="relative p-4 lg:w-1/3 w-full max-w-2xl max-h-full">
            ...
            {openIdProviders.map((provider) => (
              <button
                className={`btn-login ${provider} border border-white px-2 py-2 rounded-lg w-full`}
                onClick={() => {
...
{/* { !currentWallet && !(accounts.current?.length > 0)  && (
          <div className="flex space-x-4 justify-center">
            {openIdProviders.map((provider) => (
              <button
                className={`btn-login ${provider} border border-white p-2 rounded-lg`}
                onClick={() => {
                  beginZkLogin(provider);
                }}
                key={provider}
